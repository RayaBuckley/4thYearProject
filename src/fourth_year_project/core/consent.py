...
Authorisation answers:"May this action happen under the provider policy and intersection rule?"

Consent answers:"Will the current decision principals permit this action to proceedwithout further confirmation?"

Consent must be tied to decision provenance. It is not enough that a principalexists somewhere in the session; the principal must be one of the actualdecision principals for the action being considered."""

from future import annotations

from dataclasses import dataclass, fieldfrom enum import Enumfrom typing import FrozenSet, Iterable

from .actions import Action, ActionKindfrom .permissions import Permission, normalise_permissionfrom .principals import Principalfrom .resources import Resource

class ConsentState(str, Enum):"""Possible outcomes for a consent check."""

ALLOW = "allow"
WITHHOLD = "withhold"
REQUIRE_CONFIRMATION = "require_confirmation"

@dataclass(frozen=True, slots=True)class ConsentGrant:"""Consent provided by a principal for a restricted set of actions.
...
@dataclass(frozen=True, slots=True)class ConsentProfile:"""A consent policy for a single principal.
...
@dataclass(frozen=True, slots=True)class ConsentWitness:"""Records which principals actually consented to a specific action.
...
@dataclass(frozen=True, slots=True)class ConsentDecision:"""The result of evaluating consent for a candidate action.
...
def consent_from_profiles(action: Action[object],decision_principals: Iterable[Principal],profiles: Iterable[ConsentProfile],) -> ConsentDecision:"""Evaluate whether the action has valid consent from the current decision principals.
...
def consent_allows(action: Action[object],decision_principals: Iterable[Principal],profiles: Iterable[ConsentProfile],) -> bool:"""Convenience wrapper returning only the boolean result."""return consent_from_profiles(action, decision_principals, profiles).allowed

all = ["ConsentDecision","ConsentGrant","ConsentProfile","ConsentState","ConsentWitness","consent_allows","consent_from_profiles",]
